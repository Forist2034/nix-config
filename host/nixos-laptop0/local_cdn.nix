...
in {
  ...
  users = {
    users = { inherit proxy-user; };
    groups = { inherit proxy-group; };
  };
    ...
    config = { config, ... }: {
      ...
      services.nginx = {
        ...
        defaultListenAddresses =
          [ "0.0.0.0" "unix:/run/local_cdn/nginx/nginx.sock" ];
  ...
  local_cdn.dns = {
    ...
    config = {
      ...
      servers = let
        cloudflare = [ "cloudflare_https" "cloudflare" ];
        aliyun = [ "aliyun_https" "aliyun" ];
      ...
      in {
        main = {
          action = {
            default_action.forward = {
              upstream = [ "cloudflare_https" "cloudflare" ];
            };
            actions = [
              {
                domains = [
              ...
              (cacheProxy "cdn.sstatic.net" cloudflare)
              {
                ...
                action.forward = { upstream = aliyun; };
              }
            ];
          };
          listen = [
        ...
        cf_forward = {
          action = {
            default_action.forward = { upstream = cloudflare; };
            actions = [ ];
          };
          listen = [
            { udp = "127.0.0.100:53"; }
  ...
  systemd.services.local_cdn-dns = { wantedBy = [ "multi-user.target" ]; };
}
